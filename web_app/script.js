...
]; // fill this in with your contract's ABI //Be sure to only have one array, not two
// ============================================================
abiDecoder.addABI(abi);
// call abiDecoder.decodeMethod to use this - see 'getAllFunctionCalls' for more

var contractAddress = "0xB0f05d25e41FbC2b52013099ED9616f1206Ae21B"; // fill this in with your contract's address/hash
...
async function getCreditors(user) {
    ...
    let creditorList = [];
    // first to get creditors of the users in the past
    const functionCalls = await getAllFunctionCalls(contractAddress, "add_IOU");
    functionCalls.forEach(bill => {
        // console.log("bill from: ", bill.from);
        // console.log("calling user:", user);
        // console.log(bill.from.toLowerCase() === user.toLowerCase())
        // console.log(bill.args[0])
        if (bill.from.toLowerCase() === user.toLowerCase()) {
            creditors.add(bill.args[0].toLowerCase());    // bill.args[0] is the creditor
        }
    });
    // check if they are still the creditors now by looking up the balance
    const allCreditors = Array.from(creditors);
    // console.log("allcreditor length: ", allCreditors.length)
    for (let i = 0; i < allCreditors.length; i++) {
        // console.log("user: ", user);
        // console.log("creditor: ", allCreditors[i]);
        let amountOwed = await BlockchainSplitwise.lookup(user, allCreditors[i]);
        // console.log("amount owned: ", amountOwed);
        // console.log("true/false: ", amountOwed > 0);
        if (parseInt(amountOwed) > 0) {
            // console.log("pushing: ", allCreditors[i]);
            creditorList.push(allCreditors[i]);
        }
    }
...
async function getUsers() {
    ...
    functionCalls.forEach(bill => {
        users.add(bill.from);       // bill.from is the debtor
        users.add(bill.args[0]);    // bill.args[0] is the creditor
    });
    return Array.from(users); //return a list instead of set
}

// Get the total amount owed by the user specified by 'user'
async function getTotalOwed(user) {
    let sum = 0;
    const creditorList = await getCreditors(user);
    console.log("totalowned list length", creditorList.length);
    for (let i = 0; i < creditorList.length; i++) {
        sum += await BlockchainSplitwise.lookup(user, creditorList[i]); //.call();
    }
    return sum;
}
...
async function getLastActive(user) {
    ...
    functionCalls.forEach(bill => {
        // console.log(bill.from.toLowerCase() === user.toLowerCase())
        // console.log(bill.args[0].toLowerCase() === user.toLowerCase())
        if (parseInt(bill.from) === parseInt(user) || parseInt(bill.args[0]) === parseInt(user)) {
            // console.log(bill.t)
            timestampList.push(bill.t);
        }
    });
...
async function add_IOU(creditor, amount) {
    // find a possible path from the debtor to the creditor
    const path = await doBFS(creditor.toLowerCase(), defaultAccount.toLowerCase(), getCreditors);
    let deductAmount = amount;
    if (path == null) {
        // console.log("path is null, add IOU directly");
        // console.log("creditor is:", creditor.toLowerCase());
        // console.log("debtor is:", defaultAccount);
        await BlockchainSplitwise.connect(provider.getSigner(defaultAccount)).add_IOU(creditor.toLowerCase(), amount, [], 0);
    } else {
        for (let i = 0; i < path.length - 1; i++) {
            let amountcurr = await BlockchainSplitwise.lookup(path[i], path[i + 1]);
            if (deductAmount > amountcurr) {
                deductAmount = amountcurr;
            }
        }
        await BlockchainSplitwise.connect(provider.getSigner(defaultAccount)).add_IOU(creditor.toLowerCase(), amount, path, deductAmount);
    }
}
